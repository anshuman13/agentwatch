...
function agentRow(a) {
    const tool = a.last_tool
        ? `<span class="text-sky-300">${esc(a.last_tool)}</span>${a.last_input ? ` <span class="text-slate-500">${esc(a.last_input.slice(0, 60))}</span>` : ""}`
...
async function runAB() {
    ...
    const cov = d.criteria.map((c) => {
        ...
        const cls = (h, n) => h === n && h ? "text-run" : h ? "" : "text-red-400/80";
...
function renderTx() {
    ...
    void (async () => {
      ...
      <div id="tx-detail" class="flex-1 min-w-0"></div></div>`;
        $("#tx-list").innerHTML = list.map((a) => `<div class="tx-item px-3 py-2.5 rounded-lg cursor-pointer hover:bg-ink-line" data-id="${esc(a.id)}">
