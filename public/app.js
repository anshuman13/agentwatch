...
async function showTx(id) {
    const a = (await (await fetch(`/api/agent?id=${encodeURIComponent(id)}`)).json());
...
async function loadTxList() {
    ...
    if (TFILTER)
        q.set("project", TFILTER);
    const h = (await (await fetch(`/api/history?${q}`)).json());
    const live = (LAST?.tasks.flatMap((t) => t.agents) ?? [])
        .filter((a) => !TFILTER || a.project === TFILTER);
    const list = [...live, ...h.agents];
    const opts = ["<option value=''>all projects</option>"]
        .concat(h.projects.map((p) => `<option value="${esc(p)}"${p === TFILTER ? " selected" : ""}>${esc(proj(p))}</option>`))
        .join("");
    ...
    $("#tproj").addEventListener("change", (e) => {
