import type {
  Agent, Arm, Compare, LiveView, Status, Tokens,
} from "./types.js";
...
function ripple(e: MouseEvent, host: HTMLElement): void {
...
document.addEventListener("click", (e) => {
  const t = (e.target as HTMLElement)?.closest<HTMLElement>(".md-ripple");
  if (t) ripple(e as MouseEvent, t);
});
...
$<HTMLSelectElement>("#session").addEventListener("change", (e) => {
  SESSION = (e.target as HTMLSelectElement).value || null;
  void poll();
});
...
function statCard(value: string | number, lbl: string, icon: string): string {
...
function render(d: LiveView): void {
...
function syncHint(): void {
  const f = $<HTMLSelectElement>("#mode").value === "files";
...
async function runAB(): Promise<void> {
  ...
  syncHint();
  const a = $<HTMLInputElement>("#pa").value;
  const b = $<HTMLInputElement>("#pb").value;
  const q = new URLSearchParams(files ? { ga: a, gb: b } : { a, b });
  if (SESSION) q.set("session", SESSION);

  const res = await fetch(`/api/compare?${q.toString()}`);
  const d = (await res.json()) as Compare;
  const A: Arm = d.a;
  const B: Arm = d.b;
  ...
  const cmp = (x: number, y: number, lower = false): string =>
    x === y ? "" : (lower ? x < y : x > y) ? "win" : "";
  ...
  const cov = d.criteria.map((c) => {
    ...
    const cls = (h: number, n: number): string => (h === n && h ? "hit" : h ? "" : "miss");
...
$<HTMLSelectElement>("#mode").addEventListener("change", () => {
  syncHint();
  const pa = $<HTMLInputElement>("#pa");
  const pb = $<HTMLInputElement>("#pb");
...
function renderTx(): void {
  const A: Agent[] = (LAST && LAST.agents) || [];
  $("#tx-list").innerHTML = A.map((a) =>
    `<div class="tx-item md-ripple" data-id="${esc(a.id)}">
  ...
  $$(".tx-item").forEach((el) => {
    el.addEventListener("click", async () => {
      ...
      const d = (await res.json()) as Agent;
      $("#tx-detail").innerHTML = `
        <div class="surface">
        ...
        <div class="surface"><h2><span class="material-icon">≡</span> Steps</h2>
          <ol class="steps">${d.tools.map((t) =>
            `<li><span class="sname">${esc(t.name)}</span><code>${esc(t.input)}</code></li>`).join("")
